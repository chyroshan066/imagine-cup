...
//   const { setCategory, canAccessCategory, progress, currentCategory } =
//     useGameStore();
...
//         ${
//           isAccessible
//             ? `${colors.bg} ${colors.border} cursor-pointer hover:scale-105 hover:-translate-y-2 active:scale-95`
...
//       style={{
//         animationDelay: `${
//           ["numbers", "letters", "objects"].indexOf(category) * 100
//         }ms`,
//       }}
//     >
//       {/* Glow effect */}
//       {isAccessible && (
//         <div
//           className={`absolute inset-0 ${colors.glow} opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-3xl`}
//         />
//       )}
...
//       >
//         {isAccessible ? (
//           <div className="text-primary-foreground animate-wiggle">{icon}</div>
//         ) : (
...
"use client"; // Mandatory for hooks like useRouter and useGameStore
...
const CategoryCard = ({
  ...
  const handleClick = () => {
    if (!isAccessible) return;
    setCategory(category);
    router.push("/practice"); // Changed from navigate()
  };
